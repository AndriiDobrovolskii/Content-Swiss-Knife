 ...
 *
 * Center 3D Print "Style B" confines direct second-person address to one place: the closing CTA.
 * Everywhere else the voice is impersonal, with the machine as the grammatical subject. This
 * linter enforces that.
 *
 * WARNING SEVERITY, ON PURPOSE. The CTA is a <p class="cta"> whose heading is a plain <h2>, and a
 * heading phrased unusually enough to miss detection would put that block's legitimate «ви» into
 * the findings. As a hard gate that would fail correct text and burn repair budget; as a warning
 * it surfaces in the report and the panel and costs nothing. Promote to 'error' once the
...
/**
 * Concatenates the text of every top-level block OUTSIDE the one sanctioned zone.
 *
 * <p class="cta"> is dropped wherever it appears. The CTA's own <h2> is NOT exempted — the
 * mandated "worth buying" question form contains no second-person pronoun by construction, so a
 * hit there is a real finding rather than a slicing artifact.
 */
 ...
 * @returns one 'tov-second-person-outside-scope' warning per distinct form found outside the CTA
 */
  ...
  for (const { tag, text } of textOutsideExemptBlocks(doc)) {
    for (const match of text.matchAll(SECOND_PERSON_RE)) {
      ...
      issues.push({
        ...
        detail:
          `Direct second-person address ("${match[0]}") appears in a <${tag}> outside the CTA, ` +
          `where Style B requires an impersonal voice with the machine as the subject. Context: ` +
          `"${excerpt(text, match.index ?? 0, match[0].length)}". Rewrite it in the third person.`,
...
/**
 * Every span of Doc-authored text OUTSIDE the one sanctioned zone — the Doc-side equivalent of
 * textOutsideExemptBlocks.
 *
 * cta.text is EXCLUDED (the CTA exemption); cta.heading is INCLUDED, exactly as in the HTML
 * sibling — "The CTA's own <h2> is NOT exempted" (see the module doc above): the mandated "worth
 * buying" question form contains no second-person pronoun by construction, so a hit there is a
 * real finding, not a slicing artifact.
 *
 * Reasonably broad on purpose, mirroring the HTML walker's own breadth (it scans every top-level
 * body element outside the CTA, including the killer-specs table and the §7 spec table) — every
 ...
 * an artifact's rendered HTML with DOMParser. Same store gate, same locale gate, same sanctioned
 * zone (CTA text).
 *
  ...
  for (const { path, text } of collectScopedSpans(doc)) {
    ...
    for (const match of text.matchAll(SECOND_PERSON_RE)) {
      ...
      issues.push({
        ...
        rule: 'tov-second-person-outside-scope',
        detail:
          `Direct second-person address ("${match[0]}") appears at ${path}, outside the CTA, ` +
          `where Style B requires an impersonal voice with the machine as the subject. Context: ` +
          `"${excerpt(text, match.index ?? 0, match[0].length)}". Rewrite it in the third person.`,
        context: `${locale} — Center 3D Print ToV`,
        path,
