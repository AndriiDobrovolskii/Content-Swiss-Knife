...
- Anti-anglicism for non-English output: Ukrainian "друк" not "прінт", "ПЗ" not "софт". Keep
  established terms (filament, nozzle, extruder). Material/consumable TRADE NAMES ("Nylon 12
  Powder", "Nylon 11 GF Powder", "PA12") are proper names, not generic nouns — keep them VERBATIM
  in Latin script (see [PRODUCT NAME LOCALIZATION]), never transliterate to "нейлон 12". When
  "nylon" appears as a plain generic material word (not the trade name itself), do NOT
  transliterate it 1:1 to "нейлон" — use a natural descriptive synonym instead: "зі
  склонаповненого нейлону" for glass-filled (GF) grades, "з композитного нейлону" for composite
  grades.
...
5. COMPATIBILITY (CONDITIONAL, 30–100 words) — include ONLY if the source/datasheet provides it:
   ...
   use a <b> lead-in label: <li><b>[Aspect label]</b> [value(s)]</li>. SCOPE (global, all products):
   ONLY physical cross-compatibility — compatible materials (resins, filaments, powders) and
   hardware components (build platforms, vats/tanks, post-processing stations, other physical
   accessories) — kept narrow for internal cross-linking purposes. Never assume or invent.
   OUT OF SCOPE for this section: software, apps, drivers, file formats, OS requirements, network/
   connectivity (Wi-Fi, Ethernet, USB data transfer). If the source provides that information, put
   it in §3 FUNCTIONALITY under the "Software & automation" H2 instead — never in Compatibility.
